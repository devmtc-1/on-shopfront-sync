...
export default function IndexRoute() {
  ...
  const syncProductsToShopify = async () => {
    ...
    try {
      ...
      if (urlCategories) {
        categoriesToSync = urlCategories.split(',').map(id => id.trim()).filter(id => id);
        console.log("📋 从URL参数获取分类:", categoriesToSync);
      } else {
        categoriesToSync = ["11e96ba509ddf5a487c00ab419c1109c"]; // 默认分类
        console.log("📋 使用默认分类:", categoriesToSync);
      }
      ...
      while (hasNextPage) {
        // 关键修改：添加 categories 参数
        const params = new URLSearchParams({ 
          first: pageSize,
          categories: categoriesToSync.join(',') // 添加分类参数
        });
        
        if (cursor) params.set("after", cursor);

        console.log(`📄 获取产品页面，URL: /shopfront-products?${params.toString()}`);
      ...
      <Card sectioned>
        <TextContainer>
          ...
          {searchParams.get("categories") && (
            <p>📋 当前URL分类参数: {searchParams.get("categories")}</p>
          )}
